...
Gigachad Edition — exclusive skin for top Patreon subscribers.
"""
...
NODE_NAME = "Gigachad Model Loader"
...
def _load_gguf_model(gguf_cls, model_name, weight_dtype_str):
    ...
    if _gguf_api_variant == "new":
        try:
            result = loader.load_unet(unet_name=model_name, dequant_dtype=torch_dtype)
            return result[0]
        except Exception:
            pass
    try:
        result = loader.load_unet(unet_name=model_name)
        return result[0]
    except Exception as e:
        raise RuntimeError(f"[{NODE_NAME}] GGUF load failed: {e}") from e
...
def _apply_flux_kv_cache(model):
    try:
        ...
        log.info(f"[{NODE_NAME}] FluxKVCache applied.")
        return result[0]
    except Exception:
        pass
    try:
    ...
    def load_model(self, model_name, loader_type, weight_dtype,
        ...
        if loader_type == "GGUF":
            ...
            if model_path is None:
                raise FileNotFoundError(f"[{NODE_NAME}] Cannot find model file: {model_name}")
            model_options = {}
        ...
        if sage_attention != "disabled":
            if loader_type == "GGUF":
                log.warning(f"[{NODE_NAME}] SageAttention skipped for GGUF.")
            else:
                try:
        ...
        if flux_kv_cache:
            if loader_type == "GGUF":
                log.warning(f"[{NODE_NAME}] FluxKVCache skipped for GGUF.")
            else:
                try:
...
NODE_CLASS_MAPPINGS = {
    "GigachadModelLoader": GigachadModelLoader,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "GigachadModelLoader": "⚡ Gigachad Model Loader",
}
