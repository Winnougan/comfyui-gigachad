...
const NODE_TYPE = "GigachadPowerLoraLoader";
...
class SparkleSystem {
    ...
    draw(ctx) {
        for (const p of this.particles) {
            ...
            ctx.shadowColor = "#FFD700"; ctx.shadowBlur = 6 + p.size * 2;
...
const GOLD_CORE = "#FFD700", GOLD_BRIGHT = "#FFE866", GOLD_DEEP = "#B8860B", ELEC_CORE = "#00CFFF", ELEC_PLASMA = "#3AF5FF", ELEC_ARC = "#FFFFFF";
...
function drawEnhancedGlow(ctx, node, sparkles) {
    ...
    ctx.save();
    // Deep gold aura
    ctx.shadowColor = GOLD_DEEP; ctx.shadowBlur = 40 + breathe * 20;
    ctx.strokeStyle = GOLD_DEEP; ctx.lineWidth = 1; ctx.globalAlpha = 0.08 + breathe * 0.10;
    ctx.beginPath(); ctx.roundRect(-4, yOff-4, w+8, h+8, r+4); ctx.stroke();
    // Main gold border
    ctx.shadowColor = GOLD_CORE; ctx.shadowBlur = 20 + goldPulse * 25;
    ctx.strokeStyle = GOLD_CORE; ctx.lineWidth = 2.5; ctx.globalAlpha = 0.55 + goldPulse * 0.35;
    ctx.beginPath(); ctx.roundRect(0, yOff, w, h, r); ctx.stroke();
    // Inner bright gold rim
    ctx.shadowColor = GOLD_BRIGHT; ctx.shadowBlur = 10 + goldPulse * 12;
    ctx.strokeStyle = GOLD_BRIGHT; ctx.lineWidth = 1.2; ctx.globalAlpha = 0.35 + goldPulse * 0.45;
    ctx.beginPath(); ctx.roundRect(1.5, yOff+1.5, w-3, h-3, r); ctx.stroke();
    // Electric overlay
    ctx.shadowColor = ELEC_CORE; ctx.shadowBlur = 14 + elecPulse * 20;
    ctx.strokeStyle = ELEC_PLASMA; ctx.lineWidth = 1.0; ctx.globalAlpha = 0.10 + elecPulse * 0.25;
    ctx.beginPath(); ctx.roundRect(0.5, yOff+0.5, w-1, h-1, r); ctx.stroke();
    // Gold corner diamonds
    ctx.shadowColor = "#FFF1A0"; ctx.shadowBlur = 12; ctx.globalAlpha = 0.5 + goldPulse * 0.5;
...
const NODE_TITLE = "Gigachad Power Lora Loader";
...
function showLoraSearchDialog(currentValue, onSelect) {
  const existing = document.getElementById("gigachad-lora-search-overlay");
  if (existing) existing.remove();

  const overlay = document.createElement("div");
  overlay.id = "gigachad-lora-search-overlay";
  ...
  Object.assign(dialog.style, {
    background: "#1a1400", border: "1px solid #b8860b",
  ...
  function setSelected(i) {
    rows.forEach((r, ri) => { r.el.style.background = ri === i ? "#3a2a00" : "transparent"; });
    selectedIndex = i;
...
function showStrengthDialog(currentValue, onConfirm) {
  const existing = document.getElementById("gigachad-strength-overlay");
  if (existing) existing.remove();

  const overlay = document.createElement("div");
  overlay.id = "gigachad-strength-overlay";
  Object.assign(overlay.style, {
    position: "fixed", inset: "0", zIndex: "9999",
  ...
  Object.assign(confirmBtn.style, {
    flex: "1", padding: "7px", background: "#2a1f00", border: "1px solid #B8860B",
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      this.color   = "#2a1f00";
      this.title   = "⚡ Gigachad Power Lora Loader";
      this.bgcolor = "#1a1200";
    ...
    nodeType.prototype.onDrawForeground = function (ctx) {
      ...
      ctx.fillStyle = "#FFE866"; ctx.shadowColor = "#FFD700";
      const _t = Date.now()/1000;
      ctx.shadowBlur = 6 + (0.5 + 0.5*Math.sin(_t*(2*Math.PI/3))) * 4;
      ctx.fillStyle = GOLD_BRIGHT; ctx.shadowColor = GOLD_CORE;
ctx.fillText("⚡ GIGACHAD", this.size[0] - 52, 14);
      ctx.restore();
